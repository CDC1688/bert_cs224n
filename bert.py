...
class BertSelfAttention(nn.Module):
  ...
  def attention(self, key, query, value, attention_mask):
    ...
    attention = (query @ key.transpose(-2, -1))*(1.0 / math.sqrt(attention_head_size))  #(B, nh, T, hs) #(B, nh, hs, T) -> (B, nh, T, T) 
    attention=attention+attention_mask    #boardcast , mask output (B, nh, T, T)
    attention_prob = F.softmax(attention, dim=-1)
    attention_prob=self.dropout(attention_prob)
    V_out = attention_prob @ value     # (B, nh, T, T) x (B, nh, T, hs) -> (B, nh, T, hs)
    V_out = V_out.transpose(2,1).contiguous().view(bs, seq_len, num_attention_heads * attention_head_size) 
...
class BertLayer(nn.Module):
  ...
  def forward(self, hidden_states, attention_mask):
    ...
    x2=self.add_norm(input=hidden_states,output=attention_out,
                    dense_layer=self.attention_dense,
                    dropout=self.attention_dropout,
                    ln_layer=self.attention_layer_norm)
    x2_out=self.interm_af(self.interm_dense(x2))
    output=self.add_norm(input=x2,output=x2_out,
...
class BertModel(BertPreTrainedModel):
  ...
  def embed(self, input_ids):
    ...
    embedding_all=inputs_embeds+pos_embeds+tk_type_embeds
    embedding_output=self.embed_layer_norm(embedding_all)
